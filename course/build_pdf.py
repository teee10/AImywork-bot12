...
# 部ごとのテーマカラー
PART_COLORS = {
    "base": colors.HexColor("#2B5CE6"),   # 基礎編・ブルー
    "write": colors.HexColor("#0E9F6E"),  # ライティング・グリーン
    "image": colors.HexColor("#8B5CF6"),  # 画像生成・パープル
    "video": colors.HexColor("#E0245E"),  # 動画生成・ピンクレッド
    "ops": colors.HexColor("#F0851A"),    # 実務運用・オレンジ
    "ref": colors.HexColor("#0F766E"),    # 巻末資料・ティール
}
...
class CoverPage(Flowable):
    ...
    def draw(self):
        ...
        # 上部のカラーストライプ
        stripe_colors = [PART_COLORS["base"], PART_COLORS["write"], PART_COLORS["image"],
                         PART_COLORS["video"], PART_COLORS["ops"]]
        ...
        c.drawString(M_LEFT, PAGE_H - 96, "AI FREELANCE COMPLETE COURSE")
        c.setStrokeColor(colors.HexColor("#2B5CE6"))
        c.setLineWidth(3)
        c.line(M_LEFT, PAGE_H - 112, M_LEFT + 58, PAGE_H - 112)

        c.setFillColor(colors.white)
        c.setFont(FONT_B, 33)
        c.drawString(M_LEFT, PAGE_H - 190, "AIフリーランス")
        c.drawString(M_LEFT, PAGE_H - 236, "業務完全習得講座")

        c.setFillColor(colors.HexColor("#C6D2E8"))
        c.setFont(FONT, 12)
        c.drawString(M_LEFT, PAGE_H - 274, "ライティング／画像生成／動画生成")
        c.setFont(FONT, 10.5)
        c.setFillColor(colors.HexColor("#8FA3C8"))
        c.drawString(M_LEFT, PAGE_H - 296, "案件獲得から納品までを一冊で学ぶ、実務ノウハウ8割の教科書")

        # 3ジャンルのバッジ
        badges = [("WRITING", "AIライティング", PART_COLORS["write"]),
                  ("IMAGE", "AI画像生成", PART_COLORS["image"]),
                  ("VIDEO", "AI動画生成", PART_COLORS["video"])]
        bw = (PAGE_W - 2 * M_LEFT - 24) / 3
        by = PAGE_H - 420
        for i, (en, ja, col) in enumerate(badges):
            ...
            c.setFillColor(col)
            c.setFont(FONT_B, 8)
            c.drawString(x + 14, by + 44, en)
            c.setFillColor(colors.white)
            c.setFont(FONT_B, 12)
            c.drawString(x + 14, by + 22, ja)
        ...
        c.setFillColor(colors.HexColor("#8FA3C8"))
        c.setFont(FONT, 9.6)
        c.drawString(M_LEFT, 128, "全5部・22章／巻末資料つき　初心者から実務レベルまで最短で到達するための完全ガイド")
        c.setFont(FONT_B, 10)
        c.setFillColor(colors.white)
        c.drawString(M_LEFT, 104, "マイワーク AIスクール")
        c.restoreState()
...
def front_matter(doc: Doc, pages: dict) -> list:
    ...
    story.append(Rule(PART_COLORS["base"], thickness=2.4, space_after=14))
    intro = [
    ...
    story.append(HeadingH2("本書の構成", PART_COLORS["base"]))
    rows = [
        ["部", "内容", "身につくこと"],
    ...
    story.append(build_table(rows, PART_COLORS["base"],
                             widths=[FRAME_W * 0.22, FRAME_W * 0.45, FRAME_W * 0.33]))
    story.append(Spacer(1, 8))

    story.append(HeadingH2("紙面のルール", PART_COLORS["base"]))
    story.append(P("本文中では、内容の性質に応じて次の5種類のボックスを使い分けています。色と見出しで役割がわかるようになっています。"))
    story.append(Panel("POINT", [P("その節でいちばん大事な考え方・判断基準をまとめています。時間がないときはここだけ拾い読みしても構いません。", "boxbody")], PART_COLORS["base"]))
    ...
    story.append(Panel("学習の進め方", [
        P("**1周目**：第1部→第2部と順に読み、各章末の「まとめ」とチェックリストだけで理解度を確認する。", "boxbody"),
        P("**2周目**：自分がやりたいジャンルの部（第2〜4部）を手を動かしながら再読し、成果物を3点作る。", "boxbody"),
        P("**3周目以降**：第5部の30日ロードマップに沿って、営業と制作を同時に回す。詰まったら該当章に戻る。", "boxbody"),
    ], PART_COLORS["ops"]))
    ...
    story.append(Rule(PART_COLORS["base"], thickness=2.4, space_after=12))
